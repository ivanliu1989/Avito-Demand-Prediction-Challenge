...
feat_imp = xgb.importance(cols, model=m_xgb)
xgb.plot.importance(feat_imp, top_n = 15)
...
# Feature selection
feat_imp = feat_imp$Feature
dtest <- xgb.DMatrix(data = test[, feat_imp])
tri <- caret::createDataPartition(y, p = 0.9, list = F) %>% c()
dtrain <- xgb.DMatrix(data = train[tri, feat_imp], label = y[tri])
dval <- xgb.DMatrix(data = train[-tri, feat_imp], label = y[-tri])
cols <- colnames(train)
